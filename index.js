...
async function sendMsg(jid, text, mentions = []) {
  try {
    // Simulate typing — looks human, reduces ban risk
    await sock.sendPresenceUpdate("composing", jid);
    // Humanised delay: base on message length, capped, plus small random variance
    const typingDelay = Math.min(text.length * 20, 3000) + Math.floor(Math.random() * 500);
    await new Promise(r => setTimeout(r, typingDelay));
    await sock.sendPresenceUpdate("paused", jid);
    // Small extra pause before sending — mimics real human pause after typing
...
${italic("Couldn't fully read this file type, but it's noted! 📌")}`);
    ...
    if (hasImage && mc.imageMessage) {
      ...
      try {
        ...
        // ── If caption is a generate/edit command, do image generation ────────
        const isGenFromImg = caption && /generate.*like\s*(this|it)|make.*like\s*(this|it)|create.*like\s*(this|it)|image.*like\s*(this|it)|similar.*image|same.*style|generate.*image/i.test(caption);
        const isEditCmd    = caption && /edit.*(image|this|it)|change.*style|make it|restyle|add.*to|remove.*from|turn.*into|convert.*to|anime|cartoon|realistic|watercolor|cyberpunk|sketch/i.test(caption);

        if (isDev && (isGenFromImg || isEditCmd)) {
          await sendMsg(senderJid, `🎨 _Processing: "${caption.slice(0, 60)}"..._ ⏳
This takes 10-30 seconds!`);
          const result = await editImage(base64Img, caption, senderPhone);
          if (!result.success) {
            await sendMsg(senderJid, result.error || "❌ Image generation failed. Try again!");
          } else {
            try {
              const outBuf = result.isB64
                ? Buffer.from(result.url, "base64")
                : await downloadImageBuffer(result.url);
              devMemory.lastImage = outBuf;
              await sock.sendMessage(senderJid, {
                image:   outBuf,
                caption: `🎨 *Done!*
_"${caption.slice(0, 80)}"_
_by AlgivixAI_ ✨`,
                mimetype: result.mimeType || "image/jpeg",
              });
              recordDMConversation("assistant", `Generated image from reference: "${caption}"`);
            } catch (e) {
              await sendMsg(senderJid, "❌ Couldn't send the image: " + e.message.slice(0, 60));
            }
          }
          return;
        }
...
_Generated by AlgivixAI_ ✨`,
      ...
      if (text.startsWith("!poll")) {
        ...
        // Support both quoted: "Question" "Opt1" "Opt2"
        // and unquoted comma: Question, Opt1, Opt2
        let question, options;
        const quotedParts = raw.match(/"([^"]+)"/g);
        if (quotedParts && quotedParts.length >= 3) {
          // Quoted format — strip quotes
          question = quotedParts[0].replace(/"/g, "").trim();
          options  = quotedParts.slice(1).map(p => p.replace(/"/g, "").trim()).filter(Boolean).slice(0, 5);
        } else {
          // Try comma-separated: Question, Opt1, Opt2, Opt3
          const commaParts = raw.split(",").map(p => p.trim()).filter(Boolean);
          if (commaParts.length >= 3) {
            question = commaParts[0];
            options  = commaParts.slice(1).slice(0, 5);
          }
        }
        if (!question || !options || options.length < 2) {
          await sendMsg(jid, "📊 *Poll Usage:*\nQuoted: *!poll \"Question?\" \"Option1\" \"Option2\"*\nComma:  *!poll Question, Option1, Option2*\n_Min 2 options needed!_");
          return;
        }
        const poll = createPoll(question, options, jid, 24);

        // ── Native WhatsApp Poll (Baileys built-in) ──────────────────────────
        // This creates a REAL WhatsApp poll that people tap to vote on
        try {
          await sock.sendMessage(jid, {
            poll: {
              name:          question,
          ...
          // Also send a text summary so people know how to see results
          await sendMsg(jid,
            `📊 *Poll created!* Tap the poll above to vote.
` +
            `_Closes in 24 hours. Use *!endpoll* to close early & see results._`
          );
        } catch (pollErr) {
          // Fallback to text poll if native fails
          console.log("[Poll] Native poll failed, using text:", pollErr.message);
          await sendMsg(jid, buildPollMessage(poll));
        }
        return;
      }

      // ── Poll voting fallback — "1", "2", "3" for text polls only ─────────
      if (/^[1-5]$/.test(text.trim()) && !text.startsWith("!")) {
        const activePoll = getActivePoll(jid);
        if (activePoll) {
          const idx    = parseInt(text.trim()) - 1;
          if (idx < 0 || idx >= activePoll.options.length) {
            await sendMsg(jid, `⚠️ Invalid vote. Reply with a number between 1 and ${activePoll.options.length}.`);
            return;
          }
          const result = castVote(activePoll.id, senderPhone, idx);
          if (result.success) {
            const freshPoll  = getActivePoll(jid);
            const totalVotes = Object.keys(freshPoll?.votes || {}).length;
            const optName    = activePoll.options[idx];
            await sendTagMsg(jid,
              result.changed
                ? `🔄 @${senderPhone} changed vote to *${optName}* (${totalVotes} total)`
                : `✅ @${senderPhone} voted *${optName}* (${totalVotes} total)`,
              [senderJid]
            );
          }
          return;
...
` +
    ...
    // ── DIRECT MESSAGES ───────────────────────────────────────────────────────
      ...
      if (isDev) {
        ...
        const tl = text.toLowerCase().trim(); // available to ALL sub-blocks below
...
${digest}`;
        ...
        if (!text.startsWith("!")) {
          ...
          if (statusTrigger) {
            ...
            try {
              try {
                const sCont = [];
                if (DEVELOPER_NUM) sCont.push(DEVELOPER_NUM + "@s.whatsapp.net");
                try {
                  const gMeta = await sock.groupMetadata(TARGET_GROUP);
                  (gMeta.participants||[]).map(p=>p.id).filter(id=>id?.endsWith("@s.whatsapp.net")).slice(0,50).forEach(id=>sCont.push(id));
                } catch {}
                await sock.sendMessage("status@broadcast",
                  { text: statusTxt, backgroundColor: "#1A1A2E", font: 2 },
                  { broadcast: true, statusJidList: [...new Set(sCont)] }
                );
              } catch (se) { console.error("[Status] Post failed:", se.message); throw se; }
        ...
        // ── Mission commands — show / stop / resume ─────────────────────────
        const tl0 = text.toLowerCase().trim();
        if (/^show missions?$|^list missions?$|^my missions?$|^missions?\s*status$/i.test(tl0)) {
          const reply = listMissions();
          await sendMsg(senderJid, reply);
          recordDMConversation("assistant", reply);
          return;
        }
        const stopM = tl0.match(/^stop\s+mission\s+([\d+\s\-]{7,20})/i);
        if (stopM) {
          const ok = stopMission(stopM[1].trim());
          const r  = ok ? `✅ Mission stopped for ${stopM[1].trim()}. I'll no longer auto-reply for them.` : `❌ No active mission for that number.`;
          await sendMsg(senderJid, r);
          recordDMConversation("assistant", r);
          return;
        }
        const resumeM = tl0.match(/^resume\s+mission\s+([\d+\s\-]{7,20})/i);
        if (resumeM) {
          resumeMission(resumeM[1].trim());
          const r = `✅ Mission resumed for ${resumeM[1].trim()}. I'm watching for their replies again!`;
          await sendMsg(senderJid, r);
          recordDMConversation("assistant", r);
          return;
        }
        ...
        if (!text.startsWith("!")) {
          // ── Extract phone number — handles all formats: 08021123757, 0802 112 3757,
          // +234 802 112 3757, 234-802-1234 etc
          // Match phone numbers including international format with leading +
          const dmMatch = tl.match(
            /(?:send\s+(?:a\s+)?(?:message|msg|dm|note|text)\s+to|message|dm|whatsapp|text|ping)\s+(\+?[\d][\d\s\-+]{6,18}\d)(?:\s|,|$)/i
          ) || tl.match(
            /(?:tell|inform|notify)\s+(\+?[\d][\d\s\-+]{6,18}\d)(?:\s|,|$)/i
          );

          // Extract "about" — strip trigger word AND phone number from original text
          let dmAbout = null;
          if (dmMatch) {
            // Use matched number from regex to do precise removal
            const matchedNum = dmMatch[1]; // e.g. "+92345 4624301" or "08021123757"
            dmAbout = text
              // Strip leading trigger phrase (with or without "to")
              .replace(/^\s*(?:send\s+(?:a\s+)?(?:message|msg|dm|note|text)\s+to\s+|message\s+to\s+|message\s+|dm\s+|whatsapp\s+|text\s+|ping\s+|tell\s+|inform\s+|notify\s+)/i, "")
              // Strip the exact matched phone number (escape special chars)
              .replace(matchedNum.replace(/[+\-\s]/g, s => `\${s.trim()||"\s+"}`), "")
              // Strip leftover number chars at start (in case regex escape missed some)
              .replace(/^[\d\s\-+]{3,20}/, "")
              // Strip connector words
              .replace(/^[,\s]*(?:and\s+)?(?:telling?|saying?|about|that|to|:)?\s*/i, "")
              .trim();
          }

          if (dmMatch && dmAbout) {
            let rawNum  = dmMatch[1].replace(/[\s\-]/g, "").replace(/^\+/, ""); // strip spaces/dashes/leading +
            const about = dmAbout;

            // Smart number normalization
            if (rawNum.startsWith("0") && rawNum.length === 11) {
              rawNum = "234" + rawNum.slice(1);           // Nigerian local: 08012345678 → 2348012345678
            } else if (rawNum.startsWith("0") && rawNum.length < 11) {
              rawNum = "234" + rawNum.slice(1);           // short Nigerian number
            } else if (rawNum.length <= 7) {
              rawNum = "234" + rawNum;                    // very short — assume Nigeria
            }
            // Otherwise keep as-is: international numbers like 923454624301, 447911123456 etc.
            ...
            try {
              msgToSend = await askGroqDirect(
                "You are AlgivixAI writing a WhatsApp message on behalf of EMEMZYVISUALS DIGITALS. " +
                "OUTPUT ONLY the message text itself — nothing else. " +
                "Do NOT include any preamble, confirmation, prefix, or label like 'Message sent to' or 'Here is the message'. " +
                "Just output the raw message text, ready to send.",
                "Write a WhatsApp message about: " + (cleanAbout || about),
                []
              );
              // Strip any accidental prefixes the AI might add
              msgToSend = (msgToSend || "").trim()
                .replace(/^["']/g, "").replace(/["']$/g, "")
                .replace(/^Message sent to [\d+\s]+:\s*/i, "")
                .replace(/^Here(?:'s| is) the message[:\s]*/i, "")
                .replace(/^AlgivixAI[:\s]*/i, "");
            ...
            try {
              ...
              // ── Auto-create mission so replies get tracked & forwarded ──
              // Every time we send on boss's behalf, start tracking that contact
              createMission(rawNum, rawNum, about || text, msgToSend, senderJid);
              const trackNote = `\n\n👁️ _I'm now watching for their reply — I'll forward it to you automatically!_\n_Say *"stop mission ${rawNum}"* if you don't want follow-up._`;
              await sendMsg(senderJid, trackNote);
              recordDMConversation("assistant", "Mission tracking started for " + rawNum);
              console.log("[DM-Send] Sent to " + rawNum + " + mission created:", msgToSend.slice(0, 50));
            } catch (e) {
...
` +
              ...
              case "post_status": {
                ...
                try {
                  // Build contact list for statusJidList (required for status to appear)
                  const sJids = [];
                  if (DEVELOPER_NUM) sJids.push(DEVELOPER_NUM + "@s.whatsapp.net");
                  try {
                    const gm = await sock.groupMetadata(TARGET_GROUP);
                    (gm.participants||[]).map(p=>p.id).filter(id=>id?.endsWith("@s.whatsapp.net")).slice(0,50).forEach(id=>sJids.push(id));
                  } catch {}
                  await sock.sendMessage("status@broadcast",
                    { text: statusTxt, backgroundColor: "#1A1A2E", font: 2 },
                    { broadcast: true, statusJidList: [...new Set(sJids)] }
                  );
...
${ctxMsg}`
        ...
        // ── Intercept "generate image like this" without attachment ──────────
        if (/generate.*like\s*(this|it)|make.*like\s*(this|it)|create.*like\s*(this|it)/i.test(text)) {
          const hint = `📎 *Please attach the image first!*\n\nSend the image and in the caption write:\n_"generate an image like this"_\n\nor just describe what you want:\n_"!imagine a similar style to Lagos street art"_`;
          await sendMsg(senderJid, hint);
          recordDMConversation("assistant", hint);
          return;
        }

        // ── Default: Chat like a human — ALWAYS respond ───────────────────
        // SAFETY: if the message looks like a "send to number" command that
        // our regex missed (e.g. spaced number, different phrasing), we must
        // NOT let the AI pretend it sent — redirect to correct format instead
        const looksLikeSendCmd = /(?:send|message|dm|whatsapp|text|ping|tell|inform|notify)(?:\s+\w+){0,4}\s+([\d][\d\s\-+]{6,18}\d)/i.test(text);
        if (looksLikeSendCmd) {
          // Try one more time — strip ALL non-digits from any number-like chunk
          const numExtract = text.match(/(?:^|\s)((?:0|\+?234)[\d\s\-]{9,14})(?:\s|,|$)/i);
          if (numExtract) {
            const cleanNum = numExtract[1].replace(/\D/g, "");
            const normNum  = cleanNum.startsWith("0") && cleanNum.length === 11 ? "234" + cleanNum.slice(1) : cleanNum;
            // Extract what to say — everything after the number
            const afterNum = text.slice(text.indexOf(numExtract[1]) + numExtract[1].length)
              .replace(/^[,\s]+(?:telling?|saying?|about|that|and|in|to|:)?\s*/i, "").trim();
            if (afterNum.length > 2) {
              // Actually attempt to send
              const targetJid = normNum + "@s.whatsapp.net";
              let msgToSend;
              try {
                msgToSend = await askGroqDirect(
                  "You are AlgivixAI writing a WhatsApp message on behalf of EMEMZYVISUALS DIGITALS. " +
                  "OUTPUT ONLY the message text itself — nothing else. No preamble, no prefix, no label. Just the raw message.",
                  "Write a WhatsApp message about: " + afterNum, []
                );
                msgToSend = (msgToSend || "").trim()
                  .replace(/^["']/g, "").replace(/["']$/g, "")
                  .replace(/^Message sent to [\d+\s]+:\s*/i, "")
                  .replace(/^Here(?:'s| is) the message[:\s]*/i, "")
                  .replace(/^AlgivixAI[:\s]*/i, "");
              } catch { msgToSend = afterNum; }
              try {
                await sock.sendMessage(targetJid, { text: msgToSend });
                const reply = `✅ *Message sent to +${normNum}!*\n📨 _"${msgToSend.slice(0, 80)}${msgToSend.length > 80 ? "..." : ""}"_`;
                await sendMsg(senderJid, reply);
                recordDMConversation("assistant", reply);
              } catch (e) {
                const reply = `❌ Couldn't send to +${normNum}.\nError: ${e.message.slice(0, 60)}`;
                await sendMsg(senderJid, reply);
                recordDMConversation("assistant", reply);
              }
              return;
          ...
          // No valid number found — tell boss the correct format
          const hint = `⚠️ I couldn't find a valid phone number in that.\n\nCorrect format:\n_"send a message to 08021123757 telling him the meeting is 3PM"_\n\nYou can also write the number without spaces: *08021123757*`;
          await sendMsg(senderJid, hint);
          recordDMConversation("assistant", hint);
          return;
        }
      ...
      // ── Agent Mission Reply Handler — RUNS FIRST, even in busy mode ──────
      if (!isDev && text) {
        console.log(`[Agent] 🔍 Checking mission for: ${senderPhone} (JID: ${senderJid})`);
        // Try multiple phone formats to find the mission
        const missionPhones = [
          senderPhone,
          senderPhone.replace(/^234/, "0"),   // 2348021123757 → 08021123757
          senderJid.split("@")[0],             // raw JID number
        ];
        let mission = null;
        for (const p of missionPhones) {
          mission = getActiveMission(p);
          if (mission) { console.log(`[Agent] ✅ Found mission via "${p}"`); break; }
        }
        if (!mission) console.log(`[Agent] ❌ No mission found for ${senderPhone}`);

        if (mission && mission.status === "active") {
          console.log("[Agent] ✅ Mission reply from " + senderPhone + ":", text.slice(0, 60));
          logMessage(senderPhone, "contact", text);

          // Always forward the raw reply to boss IMMEDIATELY — no delay
          const devJid = mission.devJid || (DEVELOPER_NUM + "@s.whatsapp.net");
          const quickReport = `📡 *Reply from ${mission.targetName || senderPhone}:*\n━━━━━━━━━━━━━━━━━━━━\n_"${text}"_\n\n⏳ _Crafting response..._`;
          await sendMsg(devJid, quickReport);
          ...
          if (!isSafeMessage(cleanReply)) {
            ...
            await sendMsg(devJid, "⚠️ _I blocked my reply — it triggered the safety filter. Tell me what to say instead._");
            return;
          }
          ...
          if (isComplete) {
            ...
            await sendMsg(devJid, `✅ *Mission complete!* I've stopped tracking ${mission.targetName || senderPhone}.`);
          } else if (isPause) {
            pauseMission(senderPhone);
            await sendMsg(devJid, `⏸️ *Mission paused* — ${mission.targetName || senderPhone} seems busy.\n_Say "resume mission ${senderPhone}" to continue._`);
          } else if ((mission.log?.length || 0) % 5 === 0) {
            await sendMsg(devJid,
              `📊 *Check-in* — ${mission.log?.length || 0} messages exchanged with ${mission.targetName || senderPhone}.\n_Say "stop mission ${senderPhone}" to end._`
            );
          }
          return;
        }
      ...
      // ── Busy auto-reply — only if NOT an active mission contact ──────────
      if (!isDev && isBusy()) {
        ...
        if (DEVELOPER_NUM) {
          await sendMsg(DEVELOPER_NUM + "@s.whatsapp.net",
            `📩 *While busy, @${senderPhone} messaged:*\n_"${text}"_`
          );
        }
        return;
      }
      ...
      if (reply) {
        ...
        // Non-member, non-mission DM — politely acknowledge, forward to boss
        const nReply = "👋 Hey! I'm *AlgivixAI* by EMEMZYVISUALS DIGITALS.\n\nI work best in the *Algivix Dev Team* group.\nType *!help* to see what I can do! 🤖";
        await sendMsg(senderJid, nReply);
        // Forward to boss so he knows someone DM'd
        if (DEVELOPER_NUM) {
          await sendMsg(DEVELOPER_NUM + "@s.whatsapp.net",
            `📩 *Unknown contact DM'd:* +${senderPhone}\n_"${text.slice(0,100)}"_\n\n_They're not on any active mission._`
          );
        }
      }
    }
...
function setupCron(groupJid) {
  ...
  async function postAutoStatus(label) {
    try {
      ...
      // Post status — requires statusJidList to actually show on WhatsApp
      // Build contacts list: dev + known group members
      const statusContacts = [];
      if (DEVELOPER_NUM) statusContacts.push(DEVELOPER_NUM + "@s.whatsapp.net");
      try {
        ...
        statusContacts.push(...members);
      } catch {}
      const uniqueContacts = [...new Set(statusContacts)];

      await sock.sendMessage("status@broadcast",
        { text: statusText, backgroundColor: "#1A1A2E", font: 2 },
        { broadcast: true, statusJidList: uniqueContacts }
      );
...
for (const task of dueToday) {
  ...
  cron.schedule("0 * * * *", async () => {
    if (!TARGET_GROUP) return;
    try {
      // checkDeadlines inline — uses getDeadlineReminders from taskmanager
      const tasks = loadTasks();
      const reminders = getDeadlineReminders(tasks);
      for (const r of reminders) {
        if (r.type === "overdue") {
          await sendMsg(TARGET_GROUP, `⚠️ *Task Overdue:* ${r.task.title}\nDeadline was ${r.task.deadline}. No submission yet.`);
        } else if (r.type === "due_today") {
          await sendMsg(TARGET_GROUP, `⏰ *Due Today:* ${r.task.title}\nSubmit with *!submit <#> <link>* before midnight!`);
        } else if (r.type === "due_soon") {
          await sendMsg(TARGET_GROUP, `📅 *Due Tomorrow:* ${r.task.title}\nGet it done!`);
        }
      }
    } catch (e) { console.error("[DeadlineCheck]", e.message); }
  });
...
async function connect() {
  ...
  sock = makeWASocket({
    ...
    // ── Anti-ban: mimic a real WhatsApp Web browser session ───────────────
    browser: Browsers.macOS("Chrome"),   // looks more legit than Ubuntu
    ...
    markOnlineOnConnect:            false,   // don't broadcast presence — less detectable
    retryRequestDelayMs:            2_000,   // delay retries to avoid flooding
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "open") {
      ...
      // ── Notify dev of any active missions on reconnect ────────────────────
      // So boss always knows the bot is back and watching
      setTimeout(async () => {
        try {
          const { getMissions } = require("./agent");
          const allMissions = Object.values(getMissions ? getMissions() : {});
          const active = allMissions.filter(m => m.status === "active");
          if (active.length > 0 && DEVELOPER_NUM) {
            const devJid = DEVELOPER_NUM + "@s.whatsapp.net";
            const names  = active.map(m => `• ${m.targetName || m.targetPhone}`).join("\n");
            await sendMsg(devJid,
              `🔄 *AlgivixAI back online!*\n` +
              `I'm watching *${active.length}* active mission(s):\n${names}\n\n` +
              `_If they replied while I was down, send me their reply and I'll continue the conversation._`
            );
          }
        } catch (e) { console.log("[Reconnect] Mission notify failed:", e.message); }
      }, 8000); // wait 8s for connection to stabilize
    }
