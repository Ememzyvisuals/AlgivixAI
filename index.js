...
${italic("Couldn't fully read this file type, but it's noted! 📌")}`);
    ...
    // ── DIRECT MESSAGES ───────────────────────────────────────────────────────
      ...
      if (isDev) {
        ...
        // ── Direct natural language: post to status / send to group ────────────
        if (!text.startsWith("!")) {
          ...
          if (statusTrigger) {
            // Extract what to post — everything after trigger words, or generate fresh
            const extracted = text.replace(/post\s+(on\s+)?(your\s+)?(status|story)\s*(nah|now|please|bro|boss)?/i, "").replace(/update\s+(your\s+)?(status|story)\s*(nah|now|please|bro|boss)?/i, "").trim();
            let statusTxt = extracted;
            if (!statusTxt || statusTxt.length < 5) {
              // Generate one for him
              try {
                statusTxt = await askGroqDirect(
                  "Generate ONE short punchy WhatsApp status (max 120 chars) relevant to what we were just talking about. Use 1-2 emojis. No quotes around it.",
                  "Recent chat: " + (devMemory.conversations.slice(-3).map(c => c.content).join(" | ") || "general dev life"),
                  []
                );
                statusTxt = statusTxt?.trim().replace(/^"|"$/g, "").slice(0, 120);
          ...
          if (groupTrigger && TARGET_GROUP) {
            const extracted2 = text.replace(/send\s+(.+?\s+)?to\s+(the\s+)?group(\s+now)?/i, "$1").replace(/post\s+(.+?\s+)?to\s+(the\s+)?group/i, "$1").replace(/share\s+(.+?\s+)?to\s+(the\s+)?group/i, "$1").trim();
            const toSend = extracted2 && extracted2.length > 3 ? extracted2 : null;
            if (toSend) {
