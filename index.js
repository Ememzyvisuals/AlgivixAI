...
${italic("Couldn't fully read this file type, but it's noted! 📌")}`);
    ...
    if (isGroup) {
      ...
      // ── Task submission detection (group) ────────────────────────────────────
      const submission = detectSubmission(text);
      if (submission) {
        const taskFound = getAllTasks().find(t => t.id === parseInt(submission.taskId));
        if (taskFound) {
          recordSubmission(submission.taskId, senderPhone, submission.content);
          await sendTagMsg(jid,
            "✅ *Submission received!*\n━━━━━━━━━━━━━━━━━━━━\n" +
            "@" + senderPhone + " submitted for *" + taskFound.title + "*\n" +
            "_Will be reviewed after deadline 🔍_",
            [senderJid]
          );
          return;
        }
    ...
    // ── DIRECT MESSAGES ───────────────────────────────────────────────────────
      ...
      if (isDev) {
        ...
        if (/^(?:remove|delete)\s+task/i.test(lowerDMText)) {
          await handleRemoveTask(text, senderPhone, sendMsg);
          return;
        }

        if (lowerDMText === "list tasks" || lowerDMText === "show tasks" || lowerDMText === "my tasks") {
          await sendMsg(senderJid, formatTaskList(getAllTasks(), true));
          return;
        }

        if (lowerDMText === "post tasks to group" || lowerDMText === "send tasks to group") {
          if (TARGET_GROUP) {
            await sendMsg(TARGET_GROUP, formatTaskList(getActiveTasks()));
            await sendMsg(senderJid, "✅ *Tasks posted to the group!* 📋🔥");
          }
          return;
        }

        if (/^set goal:/i.test(lowerDMText)) {
          const goal    = text.replace(/set goal:\s*/i, "").trim();
          const tdata   = loadTasks();
          tdata.weeklyGoal = goal;
          saveTasks(tdata);
          await sendMsg(senderJid, "✅ *Weekly goal updated!*\n_" + goal + "_");
          return;
        }

        const taskUpdateMatch = text.match(/update\s+task\s+#?(\d+)\s+status:\s*(.+)/i);
        if (taskUpdateMatch) {
          const updated = updateTask(taskUpdateMatch[1], { status: taskUpdateMatch[2].trim().toLowerCase() });
          await sendMsg(senderJid, updated
            ? "✅ *Task #" + taskUpdateMatch[1] + " updated!* Status: " + updated.status
            : "❌ Task #" + taskUpdateMatch[1] + " not found!"
          );
          return;
        }
