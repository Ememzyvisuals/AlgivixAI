...
${digest}`;
        ...
        if (!text.startsWith("!")) {
          ...
          // Also try to extract "about" part from original text (not tl, to preserve case)
          let dmAbout = null;
          if (dmMatch) {
            // Strip the trigger phrase + number to get the "about" content
            dmAbout = text
              .replace(/^(?:send\s+(?:a\s+)?(?:message|msg|dm|note|text)\s+to|message\s+to|dm\s+|whatsapp\s+|text\s+|ping\s+|tell\s+|inform\s+|notify\s+)/i, "")
              .replace(/^[\d\s\-+]{7,20}/i, "")  // remove the number
              .replace(/^[,\s]+(?:telling?|saying?|about|that|and|in|to|:)?\s*/i, "")
              .trim();
          }
...
function setupCron(groupJid) {
  ...
  async function postAutoStatus(label) {
    try {
      ...
      // Post status with proper format — text with background color
      // WhatsApp shows green ring when status is posted successfully
      await sock.sendMessage("status@broadcast", {
        text: statusText,
        backgroundArgb: 0xFF1A1A2E,   // dark blue background
        font: 5,                        // sans-serif bold
      });
