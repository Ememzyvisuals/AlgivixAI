...
const {
  devMemory,
  detectSnitchTrigger, generateAISnitch, getStatusContent, postStatus,
  viewAndReactToStatus, getGreeting, getTechStory, getDailyQuote,
  getRandomHumanMessage, getReactionForMessage, reactToMessage,
  detectDisrespect, getBodyguardResponse, buildDevNotification,
...
${italic("Couldn't fully read this file type, but it's noted! 📌")}`);
    ...
    if (hasImage && mc.imageMessage) {
      ...
      try {
        ...
        if (analysis) {
          const response = buildImageResponse(analysis);
          const targetJid = isGroup ? jid : senderJid;
    ...
    if (isGroup) {
      ...
      if (!isDev && detectDisrespect(text, DEVELOPER_NUM)) {
        await sendMsg(jid, getBodyguardResponse(senderPhone));
        return;
      }
      ...
      if (!isDev && TARGET_GROUP && (
        ...
        const notif  = buildDevNotification(
          text.includes("?") ? "question" : "mention",
          { phone: senderPhone, text }
        );
        if (notif) await sendMsg(devJid, notif);
      }

      // ── Auto react to messages (15% chance) ───────────────────────────────
      if (Math.random() < 0.15) {
        const emoji = getReactionForMessage(text);
        if (emoji) await reactToMessage(sock, jid, msg.key, emoji);
      }

      // ── Always react to developer messages (60% chance) ──────────────────
      if (isDev && Math.random() < 0.6) {
        const reactions = ["fire", "heart", "thumbsup"];
        const emoji = getReactionForMessage(text) || ["fire", "heart", "thumbsup"][Math.floor(Math.random() * 3)];
        await reactToMessage(sock, jid, msg.key, emoji);
      }
    ...
    // ── DIRECT MESSAGES ───────────────────────────────────────────────────────
      ...
      if (isDev && (text.toLowerCase().startsWith("post status:") || text.toLowerCase().startsWith("post status "))) {
        ...
        const content   = await getStatusContent(statusMsg);
        const success   = await postStatus(sock, content);
...
function setupCron(groupJid) {
  ...
  cron.schedule("30 6 * * *", async () => {
    await sendMsg(groupJid, getDailyQuote("morning"));
  });
  ...
  cron.schedule("5 6 * * 1-5", async () => {
    ...
    await sendMsg(devJid, buildMorningBriefing(stats));
  });
  ...
  cron.schedule("0 13 * * 1-5", async () => {
    await sendMsg(groupJid, getDailyQuote("afternoon"));
  });
  ...
  cron.schedule("0 19 * * *", async () => {
    if (Math.random() < 0.4) await sendMsg(groupJid, getDailyQuote("evening"));
  });
...
` +
      ...
      if (DEVELOPER_NUM) {
        ...
        await sendMsg(devJid,
          `😴 ${fmt("Inactive Member Report:")}
${toTag.map(m => `• @${m.phone} (${m.hoursAgo}h absent)`).join("
")}
