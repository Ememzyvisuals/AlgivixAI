...
// ─── MODEL 3: IMAGE GENERATION — Hugging Face FLUX.1-schnell (FREE) ──────────
// FREE forever: signup at huggingface.co → Settings → Access Tokens → New Token
// No credit card. No deposit. No rate limit surprises. Returns raw image bytes.
// Model: black-forest-labs/FLUX.1-schnell (fast, high quality, open-source)
...
async function generateImage(prompt, requestedBy = "boss") {
  ...
  if (!KEY) {
    return {
      success: false,
      error:
        "🔑 *Image generation — one free setup step!*\n" +
        "━━━━━━━━━━━━━━━━━━━━\n" +
        "1. Go to *huggingface.co* → Sign up (free)\n" +
        "2. Click your profile → *Settings* → *Access Tokens*\n" +
        "3. Click *New Token* → name it anything → *Read* role\n" +
        "4. Copy the token (starts with hf_...)\n" +
        "5. In Render → Environment → add: *HF_TOKEN = hf_xxx*\n" +
        "6. Redeploy → images work! 🎨 Truly free, no credit card!",
    };
  }

  const b = getBrain();
  const enhancedPrompt = prompt + ", highly detailed, sharp focus, professional quality";
  console.log(`[ImageGen] HF FLUX generating: "${prompt.slice(0, 60)}"`);

  // Try primary model first, fallback to stable-diffusion if FLUX is cold
  const models = [
  ...
  for (const model of models) {
    try {
      const result = await tryHFModel(model, enhancedPrompt, KEY);
      if (result.success) {
        // Log to unified brain
        b.generatedImages = b.generatedImages || [];
        b.generatedImages.push({ prompt, model, requestedBy, time: Date.now() });
      ...
      console.log(`[ImageGen] ${model} failed: ${result.error?.slice(0,60)} — trying fallback...`);
    } catch (e) {
      console.log(`[ImageGen] ${model} exception: ${e.message} — trying fallback...`);
    }
  }

  return { success: false, error: "❌ Image generation failed on all models. The model may be loading — try again in 30 seconds!" };
}

async function tryHFModel(model, prompt, key) {
  return new Promise((resolve) => {
    ...
    const req  = https.request({
      ...
      headers: {
        ...
        "Authorization":  `Bearer ${key}`,
        "Content-Length": Buffer.byteLength(body),
        "x-wait-for-model": "true",  // wait if model is loading (up to 60s)
      },
      timeout: 120000,  // 2 min — HF can be slow when model is cold
    }, res => {
      const chunks = [];
      ...
      res.on("end", () => {
        ...
        if (res.statusCode === 200) {
          ...
          if (contentType.startsWith("image/")) {
            resolve({ success: true, url: buf.toString("base64"), isB64: true, mimeType: contentType, prompt });
          } else {
            // Might be JSON error
            let errMsg = "Unknown error";
            try { errMsg = JSON.parse(buf.toString())?.error || errMsg; } catch {}
            resolve({ success: false, error: errMsg });
          }
        } else if (res.statusCode === 503) {
          // Model is loading — this is normal for cold start
          let errMsg = "Model loading";
          try { const j = JSON.parse(buf.toString()); errMsg = j.error || j.estimated_time ? `Model loading (${Math.round(j.estimated_time||20)}s)` : errMsg; } catch {}
          resolve({ success: false, error: errMsg + " — try again in 20-30 seconds" });
        } else {
...
// ─── IMAGE EDITING — describe edits, bot generates new image with your changes ──
// HF doesn't do img2img via inference API easily, so we do prompt-based editing:
// We use the original image description + edit instruction as a new prompt
async function editImage(base64Image, editInstruction, requestedBy = "boss") {
  const KEY = (process.env.HF_TOKEN || "").trim();
  if (!KEY) {
    return {
      success: false,
      error: "🔑 Add *HF_TOKEN* in Render env vars to use image editing! (free at huggingface.co)",
    };
  }

  // Combine edit instruction into a generation prompt
  const editPrompt = editInstruction + ", highly detailed, sharp focus, professional quality";
  console.log(`[ImageEdit] HF editing: "${editInstruction.slice(0, 60)}"`);
  return generateImage(editPrompt, requestedBy);
}
