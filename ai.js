...
// Free, public, no rate limits, always warm. Returns image URL.
async function tryPollinations(prompt) {
  try {
    ...
    // Pollinations returns the image directly at the URL — download it
    const buf = await new Promise((resolve, reject) => {
      const get = (targetUrl, redirects = 0) => {
        if (redirects > 5) return reject(new Error("Too many redirects"));
        const client = targetUrl.startsWith("https") ? https : http;
        const req = client.get(targetUrl, { timeout: 60000 }, res => {
    ...
    if (!buf || buf.length < 5000) return null; // too small = error page
    ...
    console.log("[ImageGen] Pollinations failed:", e.message);
    return null;
  }
}

// ── Provider 2 & 3: Hugging Face Inference API (with cold-start retry) ─────────
async function tryHFModel(model, prompt, attempt = 0) {
  const KEY = (process.env.HF_TOKEN || "").trim();
  if (!KEY) return null;

  try {
    console.log(`[ImageGen] Trying HF ${model.split("/")[1]} (attempt ${attempt + 1})...`);
      ...
      const req  = https.request({
        ...
        res.on("end", () => {
          ...
          if (res.statusCode === 200 && (res.headers["content-type"] || "").startsWith("image/")) {
            console.log(`[ImageGen] ✅ HF ${model.split("/")[1]} success`);
            resolve({ success: true, url: buf.toString("base64"), isB64: true, mimeType: res.headers["content-type"], prompt });
          } else if (res.statusCode === 503 && attempt === 0) {
            // Cold start — model is loading, retry once after 20s
            let wait = 20;
            try { wait = Math.min(JSON.parse(buf.toString())?.estimated_time || 20, 40); } catch {}
            console.log(`[ImageGen] HF ${model.split("/")[1]} cold start — retrying in ${wait}s...`);
            resolve("RETRY_" + wait);
          } else {
            let err = `HTTP ${res.statusCode}`;
            try { err = JSON.parse(buf.toString())?.error || err; } catch {}
            console.log(`[ImageGen] HF ${model.split("/")[1]} failed: ${err.slice(0, 60)}`);
            resolve(null);
          }
        });
      });
    ...
    if (typeof result === "string" && result.startsWith("RETRY_")) {
      ...
      return tryHFModel(model, prompt, 1); // one retry only
    }
    return result;
  } catch { return null; }
}
